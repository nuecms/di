...
async function invokeMiddleware(
  ...
  try {
    ...
    const handler =
      (instance as MiddlewareClass | ErrorMiddlewareClass)?.use ?? instance;
    const result =
      typeof handler === 'function' ? handler.apply(instance, args) : instance;
...
async function getMiddlewareInstance(
  middleware: InjectionToken | Middleware | ErrorMiddleware
) {
  try {
