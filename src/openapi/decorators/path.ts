...
export function BodyContent(
  ...
  return (target: any, method: string, descriptor: any) => {
    const meta = getOpenApiMeta(target);
    const methodMeta = (meta[method] = meta[method] || {});
    const requestBody = (methodMeta.requestBody = methodMeta.requestBody || {
      content: {},
    });
...
export function OpenApiResponse(
  ...
  return (target: any, method: string, descriptor: any) => {
    ...
    const thisResponse = (responses[status.toString()] = responses[
      status.toString()
    ] || {
      description: '',
      content: {},
    ...
    if (!schema) {
      // we're changing the description of a response
      thisResponse.description = descriptionOrProduces;
    } else {
      // we're defining what is the schema of the response
