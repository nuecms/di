  ...
  return (target: any) => {
    (async () => {
      ...
      Object.keys(meta).forEach((methodName) => {
        ...
        getRoutes(routes, methodName).forEach(
...
function isDeprecated(url: string, pathMeta: PathMeta): boolean {
  ...
  return (
    Array.isArray(pathMeta.deprecated) && pathMeta.deprecated.indexOf(url) >= 0
  );
}
...
function getRouteParams(params: { type: number; name?: string }[]): ParamDef[] {
  return params
    ...
    .map(({ type, name }) => ({
      name,
      in: ExpressParamType[type],
      required: ExpressParamType[type] === 'path',
    }));
}
...
function getParameters(
  ...
  routeParams.forEach((param) => {
    if (
      parameters.findIndex((p) => p.name === param.name && p.in === param.in) <
      0
    ) {
