...
export class Container {
  ...
  has(injectable: Injectable): boolean {
    return (
      this.providers.has(injectable) || this.parentContainer?.has(injectable)
    );
  }
  ...
  private async resolve(
    ...
    // If the provider has a value, return it directly
    if (provider.value) {
    ...
    for (const [index, dep] of provider.deps.entries()) {
      ...
      if (depProvider) {
        resolvedDeps.push(
          await this.resolve(dep.id, dep.optional, [
            ...sequenceDeps,
            ...provider.deps,
          ])
        );
    ...
    // If a factory is provided, resolve it
    let value: unknown;
    if (provider.factory) {
      value = await provider.factory(...resolvedDeps);
    }

    // Otherwise, instantiate the class if a type is defined
    if (provider.type) {
      value = new provider.type(...resolvedDeps);
    }

    // Store the resolved value in the providers map
  ...
  private registerMultiProviders(providers: Provider[]) {
    providers.forEach((provider) => {
      const injectableId = new InjectionToken(
        injectableToString(provider.provide)
      );

      // Register main provider with multi-providers
      ...
      // Create main provider that holds multiple dependencies
      this.registerSingleProviders([
        {
  ...
  private registerClassProvider(provider: ClassProvider) {
    ...
    const optionals =
      Reflect.getMetadata(OPTIONAL_DEPS_METADATA, provider.useClass) ?? [];
