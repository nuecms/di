...
describe('Container', () => {
  ...
  describe('ValueProvider', () => {
    it('registers a provider', async () => {
      ...
      container.provide([{ provide: token, useValue: 1 }]);
  ...
  describe('FactoryProvider', () => {
    it('registers a provider', async () => {
      ...
      container.provide([{ provide: token, useFactory: () => 1 }]);
    ...
    it('replaces a provider', async () => {
      ...
      container.provide([{ provide: token, useFactory: () => 2 }]);
  ...
  describe('ClassProvider', () => {
    it('registers a provider', async () => {
      ...
      expect(await container.get(TestInjectable)).toBeInstanceOf(
        TestInjectable
      );
    });
    ...
    it('creates a provider with deps', async () => {
      ...
      container.provide([
        { provide: dep, useFactory: () => 1 }, // Factory for dep
        { provide: TestInjectable, useClass: TestInjectable }, // Class provider
      ]);

      const provider = await container.get<TestInjectable>(TestInjectable);

      expect(provider).toBeInstanceOf(TestInjectable);
      expect(provider.number).toEqual(1); // Ensure that the dependency is correctly injected
    });
    ...
    it('throws a {MissingDependencyError} error', async () => {
      ...
      try {
        await container.get<TestInjectable>(TestInjectable);
      } catch (error) {
        // console.log(error);
        expect(error).toBeInstanceOf(MissingDependencyError);
      }
  ...
  describe('With ExtraContainer', () => {
    it('registers a provider with parent container dependency', async () => {
      ...
      extraContainer.provide([{ provide: dep, useValue: 1 }]);
